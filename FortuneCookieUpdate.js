...
FortuneCookie.launch = function(){
	FortuneCookie.init = function(){
		...
		FortuneCookie.sweetForecastCache = FortuneCookie.emptySweetForecastCache();
	...
	FortuneCookie.defaultConfig = function(){
		return {
			...
			simGCs : 0,
			forecastSweet : true,
			sweetForecastMax : 1000000,
			sweetForecastChunkSize : 5000,
			colorOverride: {
	...
	FortuneCookie.getSweetForecastMax = function(){
		return Math.max(1, Math.floor(FortuneCookie.config.sweetForecastMax || 1));
	}
	
	FortuneCookie.sweetForecastLimitLabel = function(){
		var max = FortuneCookie.getSweetForecastMax();
		return Beautify(max) + ' spell' + (max == 1 ? '' : 's');
	}
	
	FortuneCookie.UpdateSweetForecastMax = function(value){
		var oldValue = FortuneCookie.getSweetForecastMax();
		var newValue = Math.max(1, Math.floor(value || 1));
		FortuneCookie.config.sweetForecastMax = newValue;
		if(oldValue != newValue) FortuneCookie.resetSweetForecastCache();
	}
	
	FortuneCookie.SetSweetForecastMaxPrompt = function(){
		var str = '<h3>Set Sweet forecast scan limit</h3><div class="block">';
		str += '<div class="listing">Maximum number of future total Grimoire spell casts to search for the next <b>Free Sugar Lump</b>.</div>';
		str += '<div class="listing"><input id="sweetForecastMaxEditor" class="option" type="text" value="' + FortuneCookie.getSweetForecastMax() + '" style="width:120px;" /> spells</div>';
		str += '</div>';
		Game.Prompt(str, [['Save', 'FortuneCookie.UpdateSweetForecastMax(parseInt(l("sweetForecastMaxEditor").value, 10)); Game.ClosePrompt(); Game.UpdateMenu();'], ['Nevermind', 'Game.ClosePrompt();']], 0);
	}
	...
	FortuneCookie.getMenuString = function(){
		...
		str += m.Header('Sweet forecast') +
				'<div class="listing">Shows the number of total Grimoire spell casts until the next Force the Hand of Fate <b>Free Sugar Lump</b> outcome. The scan is cached and processed in chunks to avoid freezing the game.</div>' +
				'<div class="listing">Current scan limit: <b>' + FortuneCookie.sweetForecastLimitLabel() + '</b></div>' +
				'<div class="listing">' + m.ToggleButton(FortuneCookie.config, 'forecastSweet', 'forecastSweetButton', 'Sweet forecast ON', 'Sweet forecast OFF', "FortuneCookie.Toggle") + '<label>Show/Hide Sweet forecast in the Force the Hand of Fate tooltip.</label></div>' +
				'<div class="listing">' +
					m.Slider('sweetForecastMaxSlider', 'Max Sweet scan', '[$]', FortuneCookie.getSweetForecastMax, "FortuneCookie.UpdateSweetForecastMax(Math.round(l('sweetForecastMaxSlider').value)); l('sweetForecastMaxSliderRightText').innerHTML = FortuneCookie.sweetForecastLimitLabel();", 1000, 2000000, 1000) + '<br>'+
				'</div>' +
				'<div class="listing">' + m.ActionButton("FortuneCookie.SetSweetForecastMaxPrompt();", 'Set exact limit') + '<label>Set an exact maximum search length.</label></div>';
	...
	FortuneCookie.Toggle = function(prefName, button, on, off, invert){
		...
		if(prefName == 'forecastSweet') FortuneCookie.resetSweetForecastCache();
	...
	//    Sweet forecast
	//***********************************
	FortuneCookie.emptySweetForecastCache = function(){
		return {
			key : '',
			result : null,
			scanning : false,
	...
	FortuneCookie.resetSweetForecastCache = function(){
		if(!FortuneCookie.sweetForecastCache) FortuneCookie.sweetForecastCache = FortuneCookie.emptySweetForecastCache();
		if(FortuneCookie.sweetForecastCache.timer) clearTimeout(FortuneCookie.sweetForecastCache.timer);
		FortuneCookie.sweetForecastCache = FortuneCookie.emptySweetForecastCache();
	}
	
	FortuneCookie.getSweetForecastContext = function(){
		var M = Game.Objects['Wizard tower'].minigame;
		if(!M || !M.spells || !M.spells['hand of fate']) return null;
		
		var spell = M.spells['hand of fate'];
		var backfire = M.getFailChance(spell) + 0.15 * FortuneCookie.getSimGCs();
		var idx = ((Game.season == 'valentines' || Game.season == 'easter' || Game.season == 'fools' || Game.season == 'halloween') ? 1 : 0);
		var maxSearch = FortuneCookie.getSweetForecastMax();
		...
		return {
			...
			key : [
				...
				idx,
				backfire.toFixed(12),
				FortuneCookie.getSimGCs(),
				Game.hasBuff('Dragonflight') ? 1 : 0,
				Game.BuildingsOwned >= 10 ? 1 : 0,
				maxSearch,
				FortuneCookie.customFateCheckerWin ? FortuneCookie.customFateCheckerWin.length : 0,
				FortuneCookie.customFateCheckerFail ? FortuneCookie.customFateCheckerFail.length : 0
			].join('|')
		};
	}
	
	FortuneCookie.getFateOutcomeRaw = function(spellCount, idx, backfire){
	...
	FortuneCookie.getFateOutcome = function(spellCount, idx, backfire){
		var outcome = FortuneCookie.getFateOutcomeRaw(spellCount, idx, backfire);
		Math.seedrandom();
		return outcome;
	}
	
	FortuneCookie.ensureSweetForecastScan = function(){
		if(!FortuneCookie.config.forecastSweet) return null;
		if(!FortuneCookie.sweetForecastCache) FortuneCookie.sweetForecastCache = FortuneCookie.emptySweetForecastCache();
		
		var ctx = FortuneCookie.getSweetForecastContext();
		if(!ctx) return null;
		
		var cache = FortuneCookie.sweetForecastCache;
		if(cache.key != ctx.key){
			if(cache.timer) clearTimeout(cache.timer);
			FortuneCookie.sweetForecastCache = FortuneCookie.emptySweetForecastCache();
			cache = FortuneCookie.sweetForecastCache;
			cache.key = ctx.key;
		}
		
		if(cache.result || cache.scanning) return cache;
		
		cache.scanning = true;
		cache.startedAt = Date.now();
		cache.nextOffset = 0;
		cache.checked = 0;
		FortuneCookie.continueSweetForecastScan();
		return cache;
	}
	
	FortuneCookie.continueSweetForecastScan = function(){
		var cache = FortuneCookie.sweetForecastCache;
		if(!cache || !cache.scanning) return;
		
		var ctx = FortuneCookie.getSweetForecastContext();
		if(!ctx || cache.key != ctx.key){
			FortuneCookie.resetSweetForecastCache();
			return;
		}
		
		var chunkSize = Math.max(100, Math.floor(FortuneCookie.config.sweetForecastChunkSize || 5000));
		var end = Math.min(ctx.maxSearch, cache.nextOffset + chunkSize);
		
		for(var offset = cache.nextOffset; offset < end; offset++){
			if(FortuneCookie.getFateOutcomeRaw(ctx.spellsCast + offset, ctx.idx, ctx.backfire) == 'Free Sugar Lump'){
				Math.seedrandom();
				cache.result = {
					found : true,
					offset : offset,
					...
					elapsedMs : Date.now() - cache.startedAt
				};
				cache.scanning = false;
				cache.finishedAt = Date.now();
				cache.timer = null;
				Game.UpdateMenu();
				return;
			}
		...
		Math.seedrandom();
		cache.nextOffset = end;
		cache.checked = cache.nextOffset;
		
		if(cache.nextOffset >= ctx.maxSearch){
			cache.result = {
				found : false,
				offset : -1,
				checked : ctx.maxSearch,
				maxSearch : ctx.maxSearch,
				elapsedMs : Date.now() - cache.startedAt
			};
			cache.scanning = false;
		...
		cache.timer = setTimeout(FortuneCookie.continueSweetForecastScan, 0);
	}
	
	FortuneCookie.sweetForecastString = function(){
		if(!FortuneCookie.config.forecastSweet) return '';
		
		var cache = FortuneCookie.ensureSweetForecastScan();
		if(!cache) return '';
		
		var limit = cache.result ? cache.result.maxSearch : FortuneCookie.getSweetForecastMax();
		var limitLabel = Beautify(limit) + ' spell' + (limit == 1 ? '' : 's');
		
		if(cache.result){
			if(cache.result.found){
				var offset = cache.result.offset;
				return '<div style="color:#DAA560;">Next Sweet: <b>' + Beautify(offset) + ' spell' + (offset == 1 ? '' : 's') + '</b> <span class="smallText">(scan limit: ' + limitLabel + ')</span></div>';
			}else{
				return '<div class="red">Next Sweet: not found <span class="smallText">(scan limit: ' + limitLabel + ')</span></div>';
			}
		}
		
		return '<div style="color:#DAA560;">Next Sweet: scanning... <b>' + Beautify(cache.checked) + '</b> / ' + Beautify(limit) + ' checked</div>';
	}
	...
	FortuneCookie.spellForecast=function(spell){
		...
		switch(spell.name){
			case loc("Force the Hand of Fate"):
				...
				spellOutcome = spellOutcome.replace('<br/>', '<span style="color:yellow;">This spell is a bit complicated. See the Options menu for an explanation.</span><br/>' + FortuneCookie.sweetForecastString()) + 
					'<table width="100%"><tr>';
				for(var i = 0; i < 2; i++)
		...
		for(var i in FortuneCookie.customSpellForecast) spellOutcome = FortuneCookie.customSpellForecast[i](spellOutcome, spell);
		return spellOutcome;
	}
